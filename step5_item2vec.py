"""
Step 5 Item2Vec: 序列嵌入增强版
================================
基于 step5_lowlr.py 增量修改:
  - Phase 0: Word2Vec 从购买序列学习商品嵌入 (32维)
  - Phase 0.5: a2v_0~a2v_31 追加到 art_feat
...
print("=" * 60)
print("Step 5 Item2Vec: 56维 (23基线 + 32 a2v + 1 v2v_sim)")
print("=" * 60)

# ============ 特征列 (23维基线 + 32 a2v + 1 v2v_sim = 56) ============
...
print(f"  特征: {len(FEAT_COLS_CLEAN)} 维 (基线23 + item2vec32 + v2v_sim1)")
print(f"  lr=0.02  rounds=2000  early_stop=100")
...
# 构建 article → 32-dim 向量字典
art2v = {aid: w2v_model.wv[aid] for aid in art_feat["article_id"]
         if aid in w2v_model.wv}
default_vec = np.zeros(32, dtype=np.float32)

# 保存 Word2Vec 模型
w2v_model.save(f"{OUTPUT_DIR}/word2vec_article.model")
print(f"  Word2Vec 模型已保存: {OUTPUT_DIR}/word2vec_article.model")
...
# ============================================================
print(f"\n{'='*60}")
print("Phase 0.5: 追加 a2v 嵌入到 art_feat")
print(f"{'='*60}")
...
# ============================================================
print(f"\n{'='*60}")
print("Phase 1: LTR 数据构建 + LightGBM LambdaRank (56维)")
print(f"{'='*60}")

print("\n[Train LTR]")
with timer("构建 Train LTR 数据"):
...
with timer("Phase 1 LightGBM 训练 (56维 + 低LR)"):
    model = lgb.train(
        params, train_ds,
...
with timer("Phase 1 验证评估"):
    ltr_ho = build_ltr_data(candidates, val_holdout_labels, cus_feat, art_feat,
                            inter_feat, item_sim, user_hist, art2v, default_vec)
    for c in FEAT_COLS_CLEAN:
        if c not in ltr_ho.columns:
            ltr_ho[c] = np.float32(0)
        elif ltr_ho[c].dtype == "float64":
    ...
    ltr_tr = build_ltr_data(candidates, val_train_labels, cus_feat, art_feat,
                            inter_feat, item_sim, user_hist, art2v, default_vec)
    for c in FEAT_COLS_CLEAN:
        if c not in ltr_tr.columns:
            ltr_tr[c] = np.float32(0)
        elif ltr_tr[c].dtype == "float64":
...
print(f"\n{'='*55}")
print(f"  Phase 1 Val 评估 (Item2Vec 56维):")
print(f"    ┌────────────────────┬──────────┬──────────┐")
print(f"    │                    │  Train   │ Holdout  │")
print(f"    ├────────────────────┼──────────┼──────────┤")
print(f"    │ 流行度 Baseline     │ {score_pop_train:.5f}  │ {score_pop_ho:.5f}  │")
print(f"    │ 56维 Item2Vec       │ {score_train_eval:.5f}  │ {score_holdout:.5f}  │")
...
final_model.save_model(f"{OUTPUT_DIR}/model_item2vec.txt")
print(f"\n模型已保存: {OUTPUT_DIR}/model_item2vec.txt")
...
# ============================================================
print(f"\n{'='*60}")
print("Phase 3: 全量推理 (56维 + Item2Vec)")
print(f"{'='*60}")
...
sub.to_csv(f"{OUTPUT_DIR}/submission_item2vec.csv", index=False)
print(f"\n提交已保存: {OUTPUT_DIR}/submission_item2vec.csv  ({len(sub):,} 行)")
...
print(f"  Word2Vec 覆盖率:                   {n_known}/{n_total} ({n_known/n_total*100:.1f}%)")
