"""
Step 5 Clean Round2: 基于 Round 2 消融结果, 仅剔除增量噪声 (Δ基线>0)
===========================================================
与 step5_clean 的区别:
  - 从 Round 2 的34维出发 (而非 Round 1 的46维)
  - 仅剔除 Δ基线>0 的特征 (剔除后 Holdout 提升), 即11维"增量噪声"
  - 保留所有 Δ基线<0 的特征 (即使绝对值小于阈值), 共23维

增量噪声特征 (11维, Δ基线>0):
  F_count                                        (客户, +0.00011)
  avg_price, sales_count, n_buyers               (商品统计, +0.00047~0.00104)
  section_name_le                                (类别编码, +0.00067)
  text_emb_2,4,12,14,17,19                       (文本嵌入, +0.00024~0.00084)

来源: step5_feature_ablation_round2.py 输出 (逐维度消融实验 Round 2)

依赖: step1 → step2 → step4 (val_candidates.pkl)

输出: output/model_clean_r2.txt / output/submission_clean_r2.csv
"""
import sys, os
sys.stdout.reconfigure(encoding='utf-8')
os.environ["LIGHTGBM_VERBOSITY"] = "-1"  # 抑制 GPU kernel 编译 warning
...
print("=" * 60)
print("Step 5 Clean Round2: 精简23维 (仅剔除增量噪声, Δ基线>0)")
print("=" * 60)

# ============================================================
# 精简后特征列定义 (23维) — 从34维中剔除11个增量噪声
# ============================================================
# 客户特征 (4维) — 剔除: F_count (+0.00011)
CUS_COLS_CLEAN = [
...
# 商品特征 (16维)
ART_COLS_CLEAN = [
    # 商品统计 (3维) — 剔除: avg_price (+0.00104), sales_count (+0.00067), n_buyers (+0.00047)
    'popularity_score', 'price_log', 'sales_log',
    # 类别编码 (4维) — 剔除: section_name_le (+0.00067)
    'product_group_name_le', 'product_type_name_le',
    'colour_group_name_le', 'index_name_le',
]
# 文本嵌入 (7维) — 保留: 0,1,6,7,15,16,18  剔除: 2,4,12,14,17,19
...
print(f"  商品统计:  3维  剔除 avg_price/sales_count/n_buyers")
print(f"  类别编码:  4维  剔除 section_name_le")
print(f"  文本嵌入:  7维  保留 0/1/6/7/15/16/18")
print(f"  交互:      {len(INTER_COLS_CLEAN)}维")
print(f"  候选:      {len(CAND_COLS_CLEAN)}维")
print(f"  总计:      {len(FEAT_COLS_CLEAN)}维 (34→23, 剔除11维增量噪声)")
...
try:
    ...
    test_y = np.zeros(10)
    test_ds = lgb.Dataset(test_X, label=test_y, group=[test_X.shape[0]])
...
with timer("验证评估"):
    # ★ 直接复用已构建的 ltr_train / ltr_holdout, 不再重复 build_ltr_data
    # ---- Holdout ----
    ltr_holdout["score"] = model.predict(ltr_holdout[FEAT_COLS_CLEAN].values)
    preds_ho = (
    ...
    # ---- Train ----
    ltr_train["score"] = model.predict(ltr_train[FEAT_COLS_CLEAN].values)
    preds_tr = (
...
print(f"\n{'='*55}")
print(f"  Val 评估对比 (23维精简 vs 34维 Round2):")
print(f"    ┌────────────────────┬──────────┬──────────┐")
...
print(f"    (参考: 34维过拟合=0.02537, 46维过拟合=0.02435)")
print(f"{'='*55}")

# ============================================================
# 保存模型
# ============================================================
model.save_model(f"{OUTPUT_DIR}/model_clean_r2.txt")
print(f"\n模型已保存: {OUTPUT_DIR}/model_clean_r2.txt")
...
sub.to_csv(f"{OUTPUT_DIR}/submission_clean_r2.csv", index=False)
print(f"\n提交已保存: {OUTPUT_DIR}/submission_clean_r2.csv  ({len(sub):,} 行)")

# ============================================================
# 汇总
# ============================================================
print(f"\n{'='*55}")
print(f"结果汇总: 23维精简模型 (仅剔除增量噪声)")
print(f"{'='*55}")
...
print(f"  特征维度:                  23 (从34精简, 仅剔除Δ>0的增量噪声)")
print(f"  剔除增量噪声 (11维):       F_count, avg_price, sales_count, n_buyers,")
print(f"                            section_name_le,")
print(f"                            text_emb_2/4/12/14/17/19")
print(f"{'='*55}")
print(f"\n完成!")
