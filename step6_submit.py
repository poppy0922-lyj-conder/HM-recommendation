...
# ============================================================
with timer("读取全量数据"):
    # 读回完整交易数据的 Parquet (step1 保存了 train_txn 和 val_txn)
...
# ============================================================
# 特征重建函数 (与 step2 相同，但应用于全量数据)
# ============================================================
from datetime import timedelta
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.decomposition import TruncatedSVD
from config import TFIDF_MAX_FEATURES, SVD_N_COMPONENTS, USER_HIST_MAX, ITEM_CF_TOP_K, ITEM_CF_MIN_CNT
...
# 保存全量特征缓存 (供消融实验等复用)
cus_feat_full.to_parquet(f"{PROCESSED_DIR}/cus_feat_full.parquet", index=False)
art_feat_full.to_parquet(f"{PROCESSED_DIR}/art_feat_full.parquet", index=False)
inter_feat_full.to_parquet(f"{PROCESSED_DIR}/inter_feat_full.parquet", index=False)
with open(f"{PROCESSED_DIR}/item_sim_full.pkl", "wb") as f:
    pickle.dump(item_sim_full, f, protocol=pickle.HIGHEST_PROTOCOL)
with open(f"{PROCESSED_DIR}/user_hist_full.pkl", "wb") as f:
    pickle.dump(user_hist_full, f, protocol=pickle.HIGHEST_PROTOCOL)
print(f"  全量特征缓存已保存")
