"""
0.02824
Step 5 Ensemble Infer (4-Model xendcg): lowlr + catboost + xgboost + xendcg
=============================================================================
四个异构模型等权平均:
...
输出: submission_ensemble_4model_xendcg.csv
"""
import sys, os
...
print("=" * 60)
print("Step 5 Ensemble (4-Model xendcg): lowlr + catboost + xgboost + xendcg")
print("=" * 60)
...
print(f"  lowlr (LambdaRank) + catboost (YetiRank) + xgboost (rank:ndcg) + xendcg (rank_xendcg)")
...
with timer(f"分批集成推理 (batch={INFER_BATCH_SIZE})"):
    for start in range(0, len(sub_cids), INFER_BATCH_SIZE):
        ...
        inf_df["score"] = (np.float64(s_lowlr) + np.float64(s_cat) +
                           np.float64(s_xgb) + np.float64(s_xendcg)) / 4.0
...
sub.to_csv(f"{OUTPUT_DIR}/submission_ensemble_4model_xendcg.csv", index=False)
print(f"\n提交已保存: {OUTPUT_DIR}/submission_ensemble_4model_xendcg.csv  ({len(sub):,} 行)")
print(f"\n完成!")
