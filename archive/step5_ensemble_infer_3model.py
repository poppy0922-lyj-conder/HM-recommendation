"""
Step 5 Ensemble Infer (3-Model): lowlr + catboost + xgboost 三模型集成推理
============================================================================
基于 step5_ensemble_infer.py, 添加 XGBoost rank:ndcg 作为第三个异构模型
...
输入: model_lowlr.txt / model_catboost.cbm / model_xgboost.json
输出: submission_ensemble_3model.csv
"""
import sys, os
...
print("=" * 60)
print("Step 5 Ensemble Infer (3-Model): lowlr + catboost + xgboost")
print("=" * 60)
...
print(f"  lowlr (LightGBM) + catboost (CatBoost YetiRank) + xgboost (XGBoost rank:ndcg) 加载完成")
...
with timer(f"分批集成推理 (batch={INFER_BATCH_SIZE})"):
    for start in range(0, len(sub_cids), INFER_BATCH_SIZE):
        ...
        inf_df["score"] = (np.float64(s_lowlr) + np.float64(s_cat) + np.float64(s_xgb)) / 3.0
        ...
        del inf_df, batch_cands, batch_preds, X_inf, dinf; gc.collect()
        print(f"  Batch {batch_num}: {len(batch_cids):,} 用户完成")

sub["prediction"] = sub["customer_id"].map(
    lambda x: " ".join(all_preds.get(x, pop12_full))
)
sub.to_csv(f"{OUTPUT_DIR}/submission_ensemble_3model.csv", index=False)
print(f"\n提交已保存: {OUTPUT_DIR}/submission_ensemble_3model.csv  ({len(sub):,} 行)")
print(f"\n完成!")
