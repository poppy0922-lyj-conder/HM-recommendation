"""
Step 5 LowLR: 低学习率 + 多树
=============================
基于 step5_train.py, 唯一改动:
  - lr: 0.05 → 0.02
  - num_boost_round: 500 → 2000
  - early_stopping_rounds: 50 → 100
  - 23维精简特征 (与"最原始"版本一致)

原理: 更小的学习率 + 更多树 = 更平滑的决策边界 = 更好泛化

输出: output/model_lowlr.txt / output/submission_lowlr.csv
"""
...
print("=" * 60)
print("Step 5 LowLR: 低学习率 + 多树 (lr=0.02, 2000轮)")
print("=" * 60)
...
print(f"  lr={params['learning_rate']}  rounds=2000  early_stop=100")
...
final_model.save_model(f"{OUTPUT_DIR}/model_lowlr.txt")
print(f"\n模型已保存: {OUTPUT_DIR}/model_lowlr.txt")
...
sub.to_csv(f"{OUTPUT_DIR}/submission_lowlr.csv", index=False)
print(f"\n提交已保存: {OUTPUT_DIR}/submission_lowlr.csv  ({len(sub):,} 行)")
